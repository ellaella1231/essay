  ...
  if (!result) {
    return (
      <div className="min-h-screen bg-slate-50 pb-10">
        ...
        <main className="px-4 py-12 max-w-lg mx-auto text-center text-slate-600">
          No grading result yet. Upload an essay first.
        </main>
      </div>
    );
  }
      ...
      <main className="px-4 py-6 max-w-lg mx-auto space-y-6">
        ...
        <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
          ...
          <div className="text-slate-700 leading-relaxed font-serif text-lg">
            {showPerfect ? result.perfect_essay : result.original_text ?? "Original transcript is unavailable for this result."}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="space-y-4">
            {result.errors.map((err, index) => (
              <div key={`${err.original_segment}-${index}`} className="bg-white rounded-2xl p-5 shadow-sm border border-red-100">
